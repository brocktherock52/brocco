/* POST /api/v1/run - SSE-streamed Claude tool-use loop with search_web + http_get.
   Cookie-rate-limited: 1 run / 24h per browser. Hard caps: 1000-char prompt, 6 steps. */
...
async function executeTool(name: string, input: Record<string, unknown>): Promise<string> {
  try {
    ...
    if (name === 'http_get') {
      ...
      const r = await fetch(url, { headers: { 'User-Agent': 'Brocco-Demo/1.0' } });
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!apiKey) {
    return Response.json(
      {
        error: 'demo offline',
        detail: 'ANTHROPIC_API_KEY not configured on server. Sign up to run agents on your own key.',
      },
      { status: 503 },
    );
  }
