...
/**
 * inference.sh-style agents grid. Each agent renders as a dense card with
 * an agent-color spotlight on hover. We pass the agent's accent color
 * through to the spotlight so each card glows in its own hue.
 */
      ...
      <div className="container-x">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 max-w-xl text-[16px] text-ink-dim">
            each agent is a markdown spec with a tool list. broadcast a goal and they fan out in
            parallel. drop-in custom agents take ~30 lines of yaml.
          </p>
        </div>
        ...
        >
          {AGENTS.map((a) => {
            ...
            const spotlight = rgb ? `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.18)` : 'rgba(167,139,250,0.18)';
            return (
              <motion.li
                key={a.name}
                variants={{
